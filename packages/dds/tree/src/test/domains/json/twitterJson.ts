import { getRandomBoolean, getRandomNumberInRange, getRandomString, getRandomStringInRange,
     getRandomUnicodeString, getRandomUnicodeStringInRange, getSizeInBytes } from "./jsonGeneratorUtilities";
/**
...
function generateTwitterStatus(type: "standard" | "retweet", includeUnicode: boolean) {
    ...
    const statusIdString = getRandomString(true, 18);
    const retweetCount = Number(getRandomStringInRange(true, 0, 7));
    const favoriteCount = Number(getRandomStringInRange(true, 0, 7));
    ...
    const twitterStatus: any = {
        ...
        text: includeUnicode ? getRandomUnicodeStringInRange(1, 200) : getRandomStringInRange(false, 1, 200),
        // source can have unicode nested in it
        source: `<a href=\"https://twitter.com/${twitterUser.screen_name}\" rel=\"nofollow\">
                ${includeUnicode ? getRandomUnicodeStringInRange(1, 30) : getRandomStringInRange(false, 1, 30)}</a>`,
    ...
    if (type === "standard") {
        // 50% probabality
        const inReplyToStatusId = Math.random() === 1 ? getRandomString(true, 18) : null;
        const inReplyToUserId = Math.random() === 1 ? getRandomString(true, 10) : null;
        ...
        twitterStatus.in_reply_to_screen_name = inReplyToUserId !== null ? getRandomStringInRange(false, 6, 30) : null;
    ...
    if (shouldAddHashtagEntity) {
        twitterStatus.entities.hashtags.push({
            text: getRandomUnicodeString(getRandomNumberInRange(1, 30)),
            indices: [
                getRandomNumberInRange(0, 199),
                getRandomNumberInRange(0, 199),
            ],
        });
    }
    ...
    if (shouldAddUserMentionsEntity) {
        const userId = getRandomString(true, 10);
        twitterStatus.entities.user_mentions.push({
            screen_name: getRandomStringInRange(false, 6, 30),
            name: getRandomUnicodeString(getRandomNumberInRange(1, 30)),
            id: Number(userId),
            id_str: userId,
            indices: [
                getRandomNumberInRange(0, 199),
    ...
    if (shouldAddMediaEntity) {
        const mediaStatusIdString = getRandomString(true, 18);
        ...
        if (shouldAddSourceIdData) {
            mediaEntity.source_status_id_str = getRandomString(true, 18);
...
function generateTwitterUser(includeUnicode: boolean): TwitterUser {
    ...
    const user: TwitterUser = {
        id: Number(userId),
        id_str: userId,
        name: includeUnicode ? getRandomUnicodeStringInRange(1, 30) : getRandomStringInRange(false, 1, 30),
        // screen names do not include unicode characters
        screen_name: getRandomStringInRange(false, 6, 30),
        location: "",
        description: includeUnicode ? getRandomUnicodeStringInRange(1, 200) : getRandomStringInRange(false, 1, 200),
        url: null,
        entities: {
        ...
        profile_link_color: getRandomString(false, 6),
        profile_sidebar_border_color: getRandomString(false, 6),
        profile_sidebar_fill_color: getRandomString(false, 6),
        profile_text_color: getRandomString(false, 6),
